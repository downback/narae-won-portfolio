...
export async function POST(request: Request) {
  try {
    ...
    if (error || !data) {
      console.error("Education create error", { message: error?.message })
      return createServerErrorResponse("Unable to create education entry.")
    }
    ...
    await insertActivityLog(supabase, {
      ...
      logContext: "Education create",
    })
    return NextResponse.json(data)
  } catch (error) {
    console.error("Education create failed", { error })
    return createServerErrorResponse(
      "Server error while creating education entry.",
    )
  }
}
