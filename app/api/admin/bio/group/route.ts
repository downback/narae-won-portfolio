...
export async function POST(request: Request) {
  try {
    ...
    if (error || !data) {
      console.error("Group show create error", { message: error?.message })
      return createServerErrorResponse("Unable to create group show entry.")
    }
    ...
    await insertActivityLog(supabase, {
      ...
      logContext: "Group show create",
    })
    return NextResponse.json(data)
  } catch (error) {
    console.error("Group show create failed", { error })
    return createServerErrorResponse("Server error while creating group show.")
  }
}
