...
export async function POST(request: Request) {
  try {
    ...
    if (error || !data) {
      console.error("Awards create error", { message: error?.message })
      return createServerErrorResponse("Unable to create awards entry.")
    }
    ...
    await insertActivityLog(supabase, {
      ...
      logContext: "Awards create",
    })
    return NextResponse.json(data)
  } catch (error) {
    console.error("Awards create failed", { error })
    return createServerErrorResponse("Server error while creating awards entry.")
  }
}
