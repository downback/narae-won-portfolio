...
- `app/api/admin/exhibitions/[id]/route.ts`
  - This file is overloaded (PATCH + DELETE + rollback orchestration + storage cleanup + additional image mutation).
  - Risk: hard to reason about failure paths, easy to regress partial-failure safety.
  - Refactor direction: keep route as transport layer, extract exhibition image/storage transaction logic into server helpers.

### HIGH

- `components/admin/exhibition/AdminExhibitionsPanel.tsx`
  - Contains multiple responsibilities: fetch/normalize data, reorder logic, modal state, CRUD dispatch.
  - Candidate split: `useExhibitionsPanelData`, `useExhibitionsReorder`, and view components.

- `components/admin/exhibition/ExhibitionUploadModal.tsx`
  - Handles too many concerns in one file: form state, preview URL lifecycle, file validation, additional image mutation UI.
  - Candidate split: file/preview handling hook + presentational subcomponents.

- `components/admin/works/WorksPanel.tsx`
  - Similar mixed concerns as exhibitions panel; strong candidate for shared admin-panel patterns.

- `app/api/admin/exhibitions/route.ts`
  - Heavy multi-step create flow with mixed DB/storage concerns.
  - Candidate split: upload transaction + exhibition persistence service layer.

### MEDIUM

- `components/admin/works/WorkUploadModal.tsx`
  - Manageable but still multi-concern; could be simplified with shared file-input/preview hooks.
- Bio CRUD route family (`app/api/admin/bio/*/route.ts` and `[id]/route.ts`)
  - Significant duplication; good candidate for small route helpers/factory (low-risk if incremental).

### LOW / CLEANUP (reusable extraction opportunities)

- `formatFileSize` / file-size constants are duplicated in upload modals.
- Drag/drop + image input handling patterns repeat across upload modals.
- Preview URL revoke patterns repeat in multiple admin files.
- Error response mapping is improved already, but some route-local patterns are still repeated.
...
- **“Too many functions in one file?”**  
  Yes. Most notable: `app/api/admin/exhibitions/[id]/route.ts`, `AdminExhibitionsPanel`, `ExhibitionUploadModal`, `WorksPanel`.

- **“Can any component be made reusable?”**  
  Yes. Highest-impact reusable candidates:
  - shared file upload/preview hook for admin modals
  - shared preview URL lifecycle hook
  - shared admin panel data/reorder hook
  - shared bio route helpers

- **“What is too long code?” (practical thresholds)**  
  For this project, I’d use:
  - **UI component:** target `< 300` lines (up to `400` if complex)
  - **API route file:** target `< 250` lines (up to `300` if complex)
  - **Utility/service file:** target `< 200` lines (up to `300` if cohesive)
  - If a file exceeds these and mixes concerns, split by responsibility first.

---

## Suggested next 3 refactors (incremental, low-risk first)

- Extract shared modal file/preview utilities (works + exhibitions upload modals).
- Split `app/api/admin/exhibitions/[id]/route.ts` into focused server helpers.
- Consolidate duplicated bio route patterns with shared handlers.

If you want, I can turn this into **Step 7 structured review report format** (Approve/Warning/Block + concrete action checklist) and write it into docs.
